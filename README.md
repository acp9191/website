...
- **App manifest** with screenshots for rich install UI, generated from a typed
...
only runs under webpack, and a bare `next build` uses Turbopack, skips it, and
still exits 0.
...
├── public/
│   ├── favicons/         # Android icons referenced by the manifest
...
│   │   ├── sitemap.ts    # Metadata route (XML + hreflang generated by Next)
...
│   ├── hooks/
│   │   └── useRevealOnScroll.ts # Shared reveal-on-scroll observer
│   ├── i18n/
